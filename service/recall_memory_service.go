	...
	if len(toInsert) > 0 {
		...
		for _, f := range toInsert {
			files = append(files, models.RecallSourceFile{
				...
				FileType: lookupFileType(req.SourceFiles, f),
				Status:   models.RecallStatusActive,
			})
		}
		if err := s.repo.BatchCreateSourceWithTx(ctx, tx, files); err != nil {
...
// DeleteSourceFile 软删源文件记录（事务）；提交后异步删对象 + 更新 md/向量
func (s *RecallMemoryService) DeleteSourceFile(ctx context.Context, userId string, req *request.DeleteSourceFileReq) error {
	...
	affected, err := s.repo.SoftDeleteSourceWithTx(ctx, tx, req.MemoryId, req.FileKey)
	if err != nil {
		tx.Rollback()
	...
	utils.LogWithCtx(ctx, "RecallMemoryService.DeleteSourceFile", "记录软删成功 | memoryId=%s fileKey=%s", req.MemoryId, req.FileKey)
...
// DeleteMemoryTheme 软删主题 + 全部源文件（事务）；提交后异步删目录 + 删向量
func (s *RecallMemoryService) DeleteMemoryTheme(ctx context.Context, userId string, req *request.DeleteMemoryThemeReq) error {
	...
	if err := s.repo.SoftDeleteWithTx(ctx, tx, req.MemoryId); err != nil {
		tx.Rollback()
		return fmt.Errorf("删除主题失败: %v", err)
	}
	if err := s.repo.SoftDeleteAllSourceWithTx(ctx, tx, req.MemoryId); err != nil {
		tx.Rollback()
		return fmt.Errorf("删除源文件失败: %v", err)
	}
	if err := tx.Commit().Error; err != nil {
	...
	utils.LogWithCtx(ctx, "RecallMemoryService.DeleteMemoryTheme", "记录软删成功 | memoryId=%s", req.MemoryId)
